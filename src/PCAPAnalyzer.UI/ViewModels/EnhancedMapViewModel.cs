...
{
    public class EnhancedMapViewModel : ReactiveObject, IActivatableViewModel
    {
    ...
    private IReadOnlyList<PacketInfo>? _allPackets;
        ...
        public EnhancedMapViewModel(IGeoIPService geoIPService, IStatisticsService statisticsService, IProtocolColorService? protocolColorService = null)
        {
        ...
        public bool ShowTop100
        {
            ...
            set
            {
                ...
                {
                    Avalonia.Threading.Dispatcher.UIThread.Post(() => _showTop100 = value);
                    return;
                }
    ...
    public void SetPackets(IReadOnlyList<PacketInfo> packets)
        ...
        public Task UpdateStatistics(NetworkStatistics statistics)
        {
            ...
            if (!Avalonia.Threading.Dispatcher.UIThread.CheckAccess())
            {
                return Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(() => UpdateStatistics(statistics));
            }
            ...
            StatusMessage = $"Public traffic: {NumberFormatter.FormatCount(publicIPPackets)} packets • {NumberFormatter.FormatBytes(publicIPBytes)} bytes • {CountriesDetected} countries • {ActiveFlows} unique flows";
        ...
        private void UpdateContinentStatistics()
        {
            ...
            var topContinents = continentStats
                ...
                .Select(c => new ContinentSummary
                {
                    ...
                    PacketCount = NumberFormatter.FormatCount(c.Value),
                    Percentage = $"({c.Value * 100.0 / totalPackets:F1}%)",
                    Color = ContinentData.Continents.TryGetValue(c.Key, out var cont2) ? cont2.PrimaryColor : "#808080"
                })
                .ToList();
        ...
        private void UpdateTopCountries()
        {
            var topCountries = CountryTrafficData
                ...
                .Select(c => new CountrySummary
                {
                    ...
                    PacketCount = NumberFormatter.FormatCount(c.Value.TotalPackets),
...
#pragma warning restore CA5394
        ...
        private void UpdateProtocolStatistics(NetworkStatistics? statistics = null)
        {
            ...
            var protocolStats = protocolCounts
                ...
                {
                    Name = p.Key,
                    Count = NumberFormatter.FormatCount(p.Value),
        ...
        private void UpdateCountryTables()
        {
            ...
            if (!Avalonia.Threading.Dispatcher.UIThread.CheckAccess())
            {
                Avalonia.Threading.Dispatcher.UIThread.Post(() => UpdateCountryTables());
                return;
            }
